...
public class SistemPenjualanParfum {
    ...
    private static void tampilkanDeskripsiProduk() {
        ...
        for (Produk produk : daftarProduk) {
            System.out.println(getDeskripsiProduk(produk));
        }
    }

    // Method overloading - polimorfisme dinamis
    private static String getDeskripsiProduk(Produk produk) {
        if (produk == null) {
            return "Produk tidak dikenali (null)";
        } else if (produk instanceof ParfumEdisiTerbatas p) {
            return "Parfum Edisi Terbatas: " + p.getNama() + " | Aroma: " + p.getAroma() + " | Edisi: " + p.getEdisi();
        } else if (produk instanceof Parfum p) {
            return "Parfum Biasa: " + p.getNama() + " | Aroma: " + p.getAroma();
        } else {
            return "Produk Umum: " + produk.getNama();
        }
    }
}
